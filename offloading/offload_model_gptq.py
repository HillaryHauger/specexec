...
from offload_model import OffloadedLayerIter
...
def load_transformer_layer_gptq(
    ...
    if hasattr(model_config, "quantization_config"):
        modules_in_block_to_quantize = (
            model_config.quantization_config.modules_in_block_to_quantize
        )
    ...
    loaded_layer_state_dict = {
        ...
        for k, v in loaded_layer_state_dict.items()
...
# GPTQ-related code:
def load_gptq_offloaded_model(
    model_name, device_size=1, main_device=torch.device("cuda:0"), max_input_length=None
):
    ...
    config = transformers.LlamaModel._autoset_attn_implementation(
        config
    )  # fix config._attn_implementation
    ...
    optimum.gptq.quantizer.GPTQQuantizer.post_init_model = (
        post_init_model_patched  # PATCH
    )
    ...
    optimum.gptq.quantizer.GPTQQuantizer.post_init_model = (
        post_init_model_original  # RESTORE
    )
    ...
    resolved_archive_file = transformers.utils.hub.cached_file(
        model_name, filename="model.safetensors"
    )

    # layer template for make_module()
    sample_layer = load_transformer_layer_gptq(
        layer_idx=0,
        model_file_path=resolved_archive_file,
        model_config=config,
        quantizer=quantizer,
        device="cpu",
    )
    ...
    def make_module():
        module = copy.deepcopy(
            sample_layer
        )  # noqa  # no need to deepcopy if only 1 layer
    ...
    for layer_idx in trange(model.config.num_hidden_layers):
        layer = load_transformer_layer_gptq(
            layer_idx=layer_idx,
            model_file_path=resolved_archive_file,
            model_config=config,
            quantizer=quantizer,
...
def post_init_model_patched(self, model):
    ...
    if self.bits == 4 and not self.disable_exllama:
        ...
        if get_device(model) == torch.device("cpu") or (
            ...
            and any(d in model.hf_device_map for d in ["cpu", "disk"])
        ):
            # raise ValueError(
    ...
    if (
        ...
        and (
            ...
            and self.max_input_length > model.config.max_length
        )
    ):
