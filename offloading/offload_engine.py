...
class OffoadingCache(torch.nn.Module):
    def __init__(
        self, make_device_module: Callable[[], ModuleWithStorage], device_size: int
    ):
        """
        ...
        self.active = False
        self.all_device_buffers = torch.nn.ModuleList(
            [self._check_module(make_device_module()) for _ in range(device_size)]
        )
        self.loaded_device_module_buffers: Deque[
            Tuple[ModuleWithStorage, Optional[ModuleUID], torch.cuda.Stream]
        ] = deque(
            [
                (module_buffer, None, torch.cuda.Stream())
                for module_buffer in self.all_device_buffers
            ]
        )
    ...
    def add_module_by_storage(self, uid: ModuleUID, storage: torch.UntypedStorage):
        ...
        assert (
            uid not in self.offloaded_storages
        ), f"Module UID {uid} already registered"
    ...
    def load_modules(
        self, *uids_to_load: ModuleUID
    ) -> Iterator[Tuple[ModuleUID, ModuleWithStorage]]:
        """
        ...
        try:
            self.active = True
            index_of_next_uid_to_load = (
                0  # index of the next info that is supposed to be loaded
            )
            for _ in range(len(self.loaded_device_module_buffers)):
                loaded_module_buffer, loaded_uid, load_stream = (
                    self.loaded_device_module_buffers.popleft()
                )
                next_uid_to_load = uids_to_load[index_of_next_uid_to_load]
                if loaded_uid != next_uid_to_load:
                    with torch.cuda.stream(load_stream):
                        loaded_module_buffer.storage.copy_(
                            self.offloaded_storages[next_uid_to_load], non_blocking=True
                        )
                self.loaded_device_module_buffers.append(
                    (loaded_module_buffer, next_uid_to_load, load_stream)
                )
            ...
            for uid_to_yield in uids_to_load:
                ...
                next_uid_to_load = uids_to_load[
                    index_of_next_uid_to_load % len(uids_to_load)
                ]
            ...
            if len(self.loaded_device_module_buffers) != len(
                self.all_device_buffers
            ):  # error handling
                print(
                    f"Recovering {len(self.all_device_buffers) - len(self.loaded_device_module_buffers)} buffers",
                    file=sys.stderr,
                )
                self.loaded_device_module_buffers: Deque[
                    Tuple[ModuleWithStorage, Optional[ModuleUID]]
                ] = deque(
                    [
                        (module_buffer, None, torch.cuda.Stream())
                        for module_buffer in self.all_device_buffers
